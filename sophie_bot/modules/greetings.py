...
from sophie_bot.modules.utils.notes import get_parsed_note_list, t_unparse_note_item, send_note
...
async def welcome(message, chat, strings):
    ...
    if 'note' not in db_item:
        db_item['note'] = {'text': "Welcome message is not enabled here for the first time\n\nLet's set a welcome message by sending /setwelcome [msg]"}
        #await message.reply("Welcomes not enabled here for the first time\n\nLet's set a welcome message by sending /setwelcome [msg]")
